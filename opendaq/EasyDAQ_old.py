...
class MyCanvas(PlotCanvas):
    def __init__(self,parent):
        self.firstZoomPoint=0
        self.secondZoomPoint=0
        PlotCanvas.__init__(self,parent)
    def OnMotion(self,event):
        self.SetCursor(wx.StockCursor(wx.CURSOR_MAGNIFIER))
    def OnMouseLeftUp(self,event):
        point =  self.PositionScreenToUser(event.GetPosition())
        self.pointB=point
        self.secondZoomPoint=self.GetClosestPoint(point,True)
        if len(self.firstZoomPoint)<=2:
            return
        if len(self.secondZoomPoint)<=2:
            return        
        self.firstZoomPoint=self.firstZoomPoint[3]
        self.secondZoomPoint=self.secondZoomPoint[3]
        
        if self.pointB[1]>self.pointA[1]:
            self.Reset()
            return
        if self.pointB[0]<self.pointA[0]:
            self.Reset()
            return
        if self.secondZoomPoint[1]==self.firstZoomPoint[1]:
            self.firstZoomPoint[1]+=1
            self.secondZoomPoint[1]-=1
        if self.secondZoomPoint[0]==self.firstZoomPoint[0]:
            self.secondZoomPoint[0]+=1
            self.firstZoomPoint[1]-=1
        self.currentXRange=self.GetXCurrentRange()
        self.currentXRatio=self.currentXRange[1]-self.currentXRange[0]
        self.currentYRange=self.GetYCurrentRange()
        self.currentYRatio=self.currentYRange[1]-self.currentYRange[0]
    
        
        self.center=[0,0]
        self.size=[0,0]
        self.size[0]=(self.secondZoomPoint[0]-self.firstZoomPoint[0])
        self.center[0]=self.size[0]/2
        self.center[0]+=self.firstZoomPoint[0]
        self.size[1]=(self.firstZoomPoint[1]-self.secondZoomPoint[1])
        self.center[1]=self.size[1]/2
        self.center[1]+=self.secondZoomPoint[1]
        
        
        self.size[0]=self.size[0]/self.currentXRatio
        self.size[1]=self.size[1]/self.currentYRatio
        if(self.size[0]<=0 or self.size[1]<=0):
            return
        self.Zoom(self.center,self.size)#self.center,self.size)
    def OnMouseLeftDown(self,event): 
        point =  self.PositionScreenToUser(event.GetPosition())
        self.pointA=point
        self.firstZoomPoint=self.GetClosestPoint(point,True)
